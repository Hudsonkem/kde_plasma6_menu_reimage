...
if [[ $EUID -eq 0 ]]; then
    bin_dir="$(qtpaths --install-prefix QT_HOST_PREFIX)/bin"
    desktop_dir="$(qtpaths --locate-dirs GenericDataLocation kio/servicemenus | sed "s/.*://")"
    doc_dir="$(qtpaths --install-prefix QT_INSTALL_PREFIX)/share/doc/kde-service-menu-reimage/"
